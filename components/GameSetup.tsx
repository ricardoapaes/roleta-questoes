...
interface GameSetupProps {
  onStartGame: (teamCount: number, bankId: string) => void;
  banks: QuestionBank[];
}
...
const GameSetup: React.FC<GameSetupProps> = ({ onStartGame, banks }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (teamCount > 0 && teamCount <= 10 && selectedBank) {
      onStartGame(teamCount, selectedBank);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-3 text-center text-xl font-bold border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white"
